...
import { initRedis, workerOptions } from '../config/queue';
...
// 创建Worker
const redisReady = initRedis().catch((error) => {
  logger.error('Redis缓存连接初始化失败，将继续处理任务但不使用缓存', error);
});
...
export const quizWorker = new Worker(
  ...
  async (job: Job<QuizJobData>) => {
    ...
    try {
      ...
      // 更新任务状态为处理中
      await updateJobProgress(job, 10, '开始解析文件...', 'active');
      ...
      // 更新进度
      await updateJobProgress(job, 30, '解析文件内容...', 'active');

      // 解析文件内容为结构化数据
      const parsedData = await parseFileToJSON(content, filePath);
      
      // 更新进度
      await updateJobProgress(job, 50, '生成刷题HTML...', 'active');

      // 调用Gemini API生成HTML
      const html = await generateQuizHTML(parsedData, orderMode);
      
      // 更新进度
      await updateJobProgress(job, 80, '保存结果...', 'active');
      ...
      if (job.id) {
        await prisma.job.update({
          ...
          data: {
            status: 'failed',
            error: errorMessage,
            progress: Math.max(0, Math.min(99, Math.floor((job.progress as any) || 0))),
            updatedAt: new Date(),
          },
        });
      }
...
async function updateJobProgress(
  ...
  progress: number,
  message: string,
  status: 'queued' | 'active' | 'completed' | 'failed' = 'active'
) {
  try {
